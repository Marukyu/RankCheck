...
{
	if (index >= 0 && (std::size_t)index < myLookupTable.size() && !myLookupTable[index].expired())
	{
		sf::IntRect rect = getImageRect(index);
		sf::Image clearImage;
		clearImage.create(rect.width, rect.height, sf::Color::Transparent);
		myTexture->update(clearImage, rect.left, rect.top);
...
{
	if (index < int(myLookupTable.size()) && index >= 0 && !myLookupTable[index].expired())
	{
		auto entry = myLookupTable[index].lock();
...
{
	myUniqueHandle = std::make_shared<UniqueHandle>(packer, id);
}
...
{
	return myUniqueHandle->packer->getImageRect(myUniqueHandle->id);
}

TexturePacker::Handle::UniqueHandle::UniqueHandle(TexturePacker* packer, NodeID id) :
	packer(packer),
	id(id)
{
}
