...
/*
 template <typename T>
 sf::Vector2<T> cprod(sf::Vector2<T> a, sf::Vector2<T> b)
 {
...
/*
 void Font::process(sf::Image img)
 {
 cwidth.resize(charLast-charFirst + 1);
 for (std::size_t i=0; i<cwidth.size(); i++)
 {
 int fx = x + (width  + spacing) * (i % charsPerLine),
 fy = y + (height + spacing) * (i / charsPerLine);
 unsigned int off;

 for (off=0; off<width; off++)
 {
 if (fx+(int)off >= (int)img.getSize().x || fy >= (int)img.getSize().y)
 break;

 if (img.getPixel(fx+off,fy) == gridColor)
 break;
 }
 ...
 img.createMaskFromColor(gridColor);
...
BitmapFont * BitmapFont::generate(sf::Image & image, unsigned int charFirst, float scale, sf::Color gridColor)
{
	...
	{
		unsigned int fx = font->x + (font->width + font->spacing) * (i % font->charsPerLine), fy = font->y
			+ (font->height + font->spacing) * (i / font->charsPerLine);
		unsigned int off;
...
{
	/*if (hasFont())
	 return mySpacing * getFont().scale;
	 else
	 */return mySpacing;
}

/*
 void Text::setTabInfo(const TabInfo * info)
 {
 if (myTabInfo != info)
 {
 myTabInfo = info;
 ...
 const Text::TabInfo * Text::getTabInfo() const
 {
 return myTabInfo;
 }
 */
...
{
	return formatPrefix() + "*" + std::string(1, num2Hex((r - 15) / 16)) + std::string(1, num2Hex((g - 15) / 16))
		+ std::string(1, num2Hex((b - 15) / 16));
}
...
std::size_t BitmapText::findCharacterAt(sf::Vector2f pos) const
{
	...
	while (pos.y > getCharacterPos(index).y + getFont().height * getFont().scale * getScale().y
		&& index < getText().length())
		++index;

	float vpos = getCharacterPos(index).y;

	for (; index <= getText().length(); ++index)
	{
		// Character passed horizontally.
		if (pos.x < (getCharacterPos(index).x + getCharacterPos(index + 1).x) / 2.0)
		{
			return index;
		}

		// Character passed vertically.
		if (vpos < getCharacterPos(index).y)
		{
			return index;
		}
	}
	...
	/*
	 ...
	 {
	 if (getCursorRect(i,1).contains(pos))
	 return i;
	 }
	...
	if (pos == getText().length())
	{
		if (isCharacterRendered(pos - 1))
			return
				getText().empty() ?
					sf::Vector2f(0, 0) :
					getCharacterPos(getText().length() - 1)
...
std::vector<sf::FloatRect> BitmapText::getCursorRectMulti(std::size_t pos, std::size_t length) const
{
	...
	{
		//ret.push_back(sf::FloatRect(0,0,0,0));
		return ret;
	}
	else if (length == 0)
	{
		ret.push_back(
			sf::FloatRect(getCharacterPos(pos).x, getCharacterPos(pos).y, getSpacing().x, getCharacterSize(pos).y));
		return ret;
	}
	...
	/*
	 ret.left = getCharacterPos(pos).x;
	 ret.top = getCharacterPos(pos).y;

	 if (length == 0 || pos >= getText().length())
	 ret.width = getSpacing().x;
	 else
	 ret.width = 0;

	 for (std::size_t i = pos; i < pos + length && i < getText().length(); ++i)
	 ret.width += getCharacterSize(i).x;

	 //ret.width *= getScale().x;
	 ret.height = getCharacterSize(pos).y;
	 */
...
{
	if (pos >= getText().length())
		return getText().empty() ? getColor() : getCharacterColor(getText().length() - 1);
...
{
	if (pos >= getText().length())
		return getText().empty() ? getColor() * sf::Color(128, 128, 128) : getCharacterColorB(getText().length() - 1);
...
float BitmapText::getRawGlyphWidth(std::size_t pos) const
{
	...
	/*
	 ...
	 {
	 float targetPos,
	 currentPos = (pos != 0 && getText()[pos-1] != '\n')
	 ? getCharacterPos(pos-1).x - getRawGlyphWidth(pos-1) - getSpacing().x
	 : 0.f,
	 defaultWidth = 100.f;

	 if (getTabInfo())
	 {
	 unsigned int n = std::count(getText().begin(), getText().begin() + pos, '\t');
	 if (n < getTabInfo()->tabColumns.size())
	 targetPos = getTabInfo()->tabColumns[n];
	 else
	 targetPos = (int(currentPos / getTabInfo()->tabWidth)+1) * float(getTabInfo()->tabWidth);
	 }
	 else
	 targetPos = (int(currentPos / defaultWidth)+1) * float(defaultWidth);

	 return std::max(getSpacing().x, targetPos - currentPos);
	 }
	 */
...
sf::Vector2f BitmapText::getCharacterSize(std::size_t pos) const
{
	...
	if (!isCharacterRendered(pos))
		return (sf::Vector2f(getSpacing().x, getFont().height) * getFont().scale) * getScale() * myMaxSizeScale;
	return sf::Vector2f(getGlyphOffset(pos), getFont().height * getFont().scale) * getScale() * myMaxSizeScale;
}

static const sf::Color TextColors[] =  // default text color palette, sorted by rainbowness... err hue. sorta.
...
void BitmapText::update()
{
	...
	sf::Vector2f charPos(0, 0), charSize(0, getFont().height * getFont().scale), texPos(0, 0), texSize(0, 0), nextOff(0,
		0);
	...
	for (std::size_t i = 0; i < getText().length(); ++i)
	{
		...
		sf::Vertex vTL(sf::Vector2f(charPos.x, charPos.y), curColor, sf::Vector2f(texPos.x, texPos.y)), vTR(
			sf::Vector2f(charPos.x + charSize.x, charPos.y), curColor, sf::Vector2f(texPos.x + texSize.x, texPos.y)),
			vBR(sf::Vector2f(charPos.x + charSize.x, charPos.y + charSize.y), curColor * colMul,
				sf::Vector2f(texPos.x + texSize.x, texPos.y + texSize.y)), vBL(
				sf::Vector2f(charPos.x, charPos.y + charSize.y), curColor * colMul,
				sf::Vector2f(texPos.x, texPos.y + texSize.y));
...
{
	/*
	 // undo previously applied maximum size.
	 for (std::size_t i = myShadowOffset; i < myVertices.getVertexCount(); ++i)
	 {
	 myVertices[i].position.x /= myMaxSizeScale.x;
	 myVertices[i].position.y /= myMaxSizeScale.y;
	 }
	 */
...
void BitmapText::draw(sf::RenderTarget & target, sf::RenderStates states) const
{
	...
	if (myMaxSizeBehavior == Clip)
	{
		...
		drawClipped(myVertices, target, states,
			origTransform.transformRect(sf::FloatRect(0, 0, getMaxSize().x, getMaxSize().y)));
	}
	else
