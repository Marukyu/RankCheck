...
struct BitmapFont
{
	...
	//bool smoothTex;					 // IN:  use bilinear filtering?
	//sf::Color gridColor;				// IN:  color for the grid and character size delimiters; gets removed.

	//void process(sf::Image img);		/// processes the font from a new loaded image.
...
public:
	...
	void setFont(const BitmapFont * font);			/// changes the font.
	...
	/*
	 struct TabInfo {
	 TabInfo() : tabColumns(), tabWidth(16.f) {}

	 std::vector<float> tabColumns;	  // positions to skip to for each instance of the tab character.
	 float tabWidth;					 // default width of tab characters beyond the column vector.
	 };

	 void setTabInfo(const TabInfo * info);	  /// sets the pointer to the struct specifying how to handle tab characters.
	 const TabInfo * getTabInfo() const;		 /// gets the above.
	 */
...
private:
	...
	std::string myString;
	const BitmapFont * myFont;

	//const TabInfo * myTabInfo;
