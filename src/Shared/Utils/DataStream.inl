...
template<typename TK> DataStream & DataStream::operator<<(const std::unordered_set<TK> & data)
{
	addIndexType(data.size());

	for (typename std::unordered_set<TK>::const_iterator it = data.begin(); it != data.end(); ++it)
		*this << *it;
...
template<typename TK> DataStream & DataStream::operator>>(std::unordered_set<TK> & data)
{
	data.clear();
	sf::Uint32 size = extractIndexType();
