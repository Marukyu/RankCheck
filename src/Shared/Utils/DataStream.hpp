...
public:
	...
	template<typename TK> DataStream & operator<<(const std::unordered_set<TK> & data);
	...
	template<typename TK> DataStream & operator>>(std::unordered_set<TK> & data);
...
private:
	...
	bool myIsBuffered = false;
	SizeType myBufferStart = 0;

	bool myIsFile = false;
	bool myIsOpen = false;
	bool myIsOutputEnabled = false;

	sf::Uint8 myIndexSize = 4;
};
